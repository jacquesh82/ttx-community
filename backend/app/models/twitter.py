...
class TwitterPost(Base):
    ...
    reply_to_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("twitter_posts.id", ondelete="SET NULL"), nullable=True, index=True)
    quote_of_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("twitter_posts.id", ondelete="SET NULL"), nullable=True)
    ...
    replied_to = relationship("TwitterPost", foreign_keys=[reply_to_id], remote_side=[id], backref="replies")
    quoted_post = relationship("TwitterPost", foreign_keys=[quote_of_id], remote_side=[id])
